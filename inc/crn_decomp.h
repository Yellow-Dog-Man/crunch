...
#define LOG(...) __android_log_print(ANDROID_LOG_DEBUG, "Unity", __VA_ARGS__);
#else
#define LOG(...)
#endif
...
DLL_EXPORT uint32 DLL_API crnd_crn_format_to_fourcc(crn_format fmt) {
  switch (fmt) {
    case cCRNFmtDXT1:
...
DLL_EXPORT crn_format DLL_API crnd_get_fundamental_dxt_format(crn_format fmt) {
  switch (fmt) {
    case cCRNFmtDXT5_CCxY:
    case cCRNFmtDXT5_xGxR:
    case cCRNFmtDXT5_xGBR:
...
DLL_EXPORT uint32 DLL_API crnd_get_crn_format_bits_per_texel(crn_format fmt) {
  switch (fmt) {
    case cCRNFmtDXT1:
    case cCRNFmtDXT5A:
    case cCRNFmtETC1:
...
DLL_EXPORT uint32 DLL_API crnd_get_bytes_per_dxt_block(crn_format fmt) {
...
DLL_EXPORT bool DLL_API crnd_validate_file(const void* pData, uint32 data_size, crn_file_info* pFile_info) {
  if (pFile_info) {
...
DLL_EXPORT bool DLL_API crnd_get_texture_info(const void* pData, uint32 data_size, crn_texture_info* pInfo) {

	if ((!pData) || (data_size < sizeof(crn_header)) || (!pInfo))
	{
		//LOG("Invalid crn_header size. Provided: %d Actual: %d", data_size, sizeof(crn_header));
		return false;
	}

	if (pInfo->m_struct_size != sizeof(crn_texture_info))
	{
		//LOG("Invalid crn_texture_info size. Provided: %d Actual: %d. Enum Size: %d", pInfo->m_struct_size, sizeof(crn_texture_info), sizeof(crn_format));
		return false;
	}
...
DLL_EXPORT const void* DLL_API crnd_get_level_data(const void* pData, uint32 data_size, uint32 level_index, uint32* pSize) {
  if (pSize)
    *pSize = 0;
...
DLL_EXPORT uint32 DLL_API crnd_get_segmented_file_size(const void* pData, uint32 data_size) {
  if ((!pData) || (data_size < cCRNHeaderMinSize))
    return false;
...
DLL_EXPORT bool DLL_API crnd_create_segmented_file(const void* pData, uint32 data_size, void* pBase_data, uint base_data_size) {
  if ((!pData) || (data_size < cCRNHeaderMinSize))
    return false;
...
DLL_EXPORT crnd_unpack_context DLL_API crnd_unpack_begin(const void* pData, uint32 data_size) {
  if ((!pData) || (data_size < cCRNHeaderMinSize))
    return NULL;
...
DLL_EXPORT bool DLL_API crnd_get_data(crnd_unpack_context pContext, const void** ppData, uint32* pData_size) {
  if (!pContext)
    return false;
...
DLL_EXPORT bool DLL_API crnd_unpack_level(
...
DLL_EXPORT bool DLL_API crnd_unpack_end(crnd_unpack_context pContext) {
  if (!pContext)
    return false;
